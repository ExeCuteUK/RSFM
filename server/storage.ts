  ...
  async createImportShipment(insertShipment: InsertImportShipment): Promise<ImportShipment> {
    ...
    if (shipment.rsToClear) {
      ...
      const clearance: CustomClearance = {
        ...
        clearanceAgent: shipment.clearanceAgent,
        mrn: shipment.mrn,
        incoterms: null,
        customerReferenceNumber: shipment.customerReferenceNumber,
        supplierName: shipment.supplierName,
        deliveryAddress: null,
        haulierName: shipment.haulierName,
        haulierContactName: shipment.haulierContactName,
        haulierEmail: shipment.haulierEmail,
        haulierReference: shipment.haulierReference,
  ...
  async updateImportShipment(id: string, updates: Partial<InsertImportShipment>): Promise<ImportShipment | undefined> {
    ...
    if (updated.rsToClear === true && !existing.linkedClearanceId) {
      ...
      const clearance: CustomClearance = {
        ...
        clearanceAgent: updated.clearanceAgent,
        mrn: updated.mrn,
        incoterms: null,
  ...
  async updateImportShipment(id: string, updates: Partial<InsertImportShipment>): Promise<ImportShipment | undefined> {
    ...
    if (finalRsToClear === true && !existing.linkedClearanceId) {
      ...
      const [clearance] = await db.insert(customClearances).values({
        ...
        clearanceAgent: updatedShipment.clearanceAgent,
        mrn: updatedShipment.mrn,
        incoterms: null,
  ...
  async updateExportShipment(id: string, updates: Partial<InsertExportShipment>): Promise<ExportShipment | undefined> {
    ...
    if (updates.exportClearanceAgent === "R.S" && existing.exportClearanceAgent !== "R.S" && !updated.linkedClearanceId) {
      const [clearance] = await db.insert(customClearances).values({
        ...
        sendHaulierEadStatusIndicator: 1,
        sendHaulierClearanceDocStatusIndicator: 1,
        sendEntryToCustomerStatusIndicator: 1,
