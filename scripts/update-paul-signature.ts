...
async function updateSignature() {
  console.log("Updating Paul's signature...");
  
  await db.update(users)
    .set({ emailSignature: defaultSignature })
    .where(eq(users.username, "Paul Shearing"));
  
  console.log("Signature updated! Length:", defaultSignature.length);
  process.exit(0);
}

updateSignature().catch(console.error);
